...
# 控制特定房间的灯光开关
file_path = r"E:\room_flask\templates\smarthome\bedroom.html"
...
def process_command(command):
    ...
    """
    try:
        url = "http://127.0.0.1:5000/set_device"
        if command == "turn on light bedroom":
            data = {
                "room_id": 1,  # Bedroom 的 room_id
                "device_type_id": 1,  # 假设灯的类型 ID 是 1
                "data": {"status": "On"}
            }
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to update device status: {response.json()}")
